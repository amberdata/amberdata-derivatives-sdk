...
class EndpointVolatilitySVIAltcoinsTableTestCase(BaseTestCase):
    # pylint: disable-next=arguments-differ
    def setUp(self):
        super().setUp(function_name='get_volatility_svi_altcoins_table')
    ...
    def test_default_timeformat_hr(self):
        response = self.call_endpoint(timeFormat='hr')
        self.validate_response_schema(response, schema=self.schema)
        self.validate_response_200(response, min_elements=20)
    ...
    def test_default_timeformat_iso(self):
        response = self.call_endpoint(timeFormat='iso')
        self.validate_response_schema(response, schema=self.schema)
        self.validate_response_200(response, min_elements=20)
